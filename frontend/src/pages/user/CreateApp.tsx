...
const FEATURES = [
  { key: 'push_notifications', label: 'Push Notifications', description: 'Send real-time push notifications to users via Firebase Cloud Messaging (FCM). Broadcast campaigns from the dashboard, or per-device targeted push: enable the JS Bridge too and your page can read each device's FCM token to notify individual users from your own backend.', helpUrl: 'https://firebase.google.com/docs/cloud-messaging', icon: Bell },
  ...
  { key: 'js_bridge', label: 'JS Bridge', description: 'JavaScript-to-native bridge for bi-directional communication. Call native Android functions from your website JavaScript — including reading the device's FCM push token so you can send per-device targeted notifications from your own backend.', helpUrl: 'https://developer.android.com/develop/ui/views/layout/webapps/webview', icon: Code2 },
