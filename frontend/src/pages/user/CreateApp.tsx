...
const FEATURES = [
  { key: 'push_notifications', label: 'Push Notifications', description: 'Send real-time push notifications to users via Firebase Cloud Messaging (FCM). Broadcast campaigns from the dashboard, or per-device targeted push: enable the JS Bridge too and your page can read each device FCM token to notify individual users from your own backend.', helpUrl: 'https://firebase.google.com/docs/cloud-messaging', icon: Bell },
  ...
  { key: 'js_bridge', label: 'JS Bridge', description: 'JavaScript-to-native bridge for bi-directional communication. Call native Android functions from your website JavaScript — including reading the device FCM push token so you can send per-device targeted notifications from your own backend.', helpUrl: 'https://developer.android.com/develop/ui/views/layout/webapps/webview', icon: Code2 },
  { key: 'screenshot_prevention', label: 'Screen Capture Protection', description: 'Block screenshots and screen recording of your app — the video is blanked. Also stops third-party recorder apps from capturing your audio, and on Android 15+ automatically mutes the audio while a screen recording is active, so recordings capture neither picture nor sound. Ideal for protecting paid or private content such as courses and lectures. (No app can stop someone recording the screen acoustically with a separate device.)', helpUrl: 'https://developer.android.com/reference/android/view/WindowManager.LayoutParams#FLAG_SECURE', icon: ShieldOff },
...
const ANDROID_PLANS_FALLBACK: Plan[] = [
  ...
  { id: 'android-paid', name: 'Paid', slug: 'android-paid', price_inr: 373500, price_usd: 4500, billing_type: 'one_time', max_apps: 1, sort_order: 2, is_active: true, platform: 'android', description: 'All features, one-time payment', features: {} },
]
...
const DESKTOP_PLANS_FALLBACK: Plan[] = [
  ...
  { id: 'desktop-paid', name: 'Paid', slug: 'desktop-paid', price_inr: 373500, price_usd: 4500, billing_type: 'one_time', max_apps: 1, sort_order: 5, is_active: true, platform: 'desktop', description: 'Full desktop app, one-time', features: {} },
]
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {wizard.selectedPlatforms.includes('android') && (
              ...
              >
                ...
                <div className="mt-1">
                  <span className="text-2xl font-bold text-indigo-700">$50</span>
            ...
            {publishServiceSuccess ? (
              ...
              <>
                ...
                <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
                  ...
                  <p>Our team will contact you shortly. You'll need to add <strong>sohamsmulay@gmail.com</strong> / <strong>support@websitetoapp.app</strong> to your Play Console with <strong>Admin / Release Manager</strong> permissions.</p>
                </div>
                <button
                  onClick={async () => {
                    ...
                    try {
                      const order = await createRazorpayOrder({ amount: 5000, currency: 'USD', receipt: `publish_${Date.now()}`, notes: { name: publishForm.name, email: publishForm.email, appName: publishForm.appName, websiteUrl: publishForm.websiteUrl, notes: publishForm.notes, service: 'play-store-publish' } })
                      const options = {
                ...
                >
                  {publishServiceLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Store className="w-4 h-4" /> Pay $50 &amp; Submit Request</>}
                </button>
              </>
            )}
          </div>
