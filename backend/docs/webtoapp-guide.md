...
* **AdMob Ads** — Banner, interstitial, and rewarded ads. Requires AdMob App ID and the matching Ad Unit IDs (App ID + Banner / Interstitial / Rewarded fields in the builder). Rewarded ads are triggered from your website via the JavaScript Bridge: call `WebToApp.showRewardedAd('yourCallback')`, and your global JS function is called as `yourCallback(rewarded, reason)` — `rewarded` is `true` only after the reward is earned (so points are granted only on a completed view), otherwise `false` with a `reason` string explaining why (`not_ready`, `no_rewarded_id`, `load_failed(code): msg`, `dismissed_no_reward`, `show_failed: …`). Callbacks that read only the first argument keep working. Use `WebToApp.isRewardedReady()` (returns a boolean) to enable your "Watch Ad" button only when an ad is loaded — ads preload in the background at launch, so a tap immediately after start may return `not_ready`. `WebToApp.showInterstitial()` is also available. Tip: a brand-new AdMob rewarded unit often returns no ad (`load_failed(3)` = no fill) for hours/days until the app is approved in your AdMob account; test your integration with Google's test rewarded unit `ca-app-pub-3940256099942544/5224354917`. For tamper-proof reward granting, use AdMob Server-Side Verification (SSV) in addition to the in-app callback.
